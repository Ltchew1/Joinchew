...
module.exports = async (req, res) => {
  ...
  // while verifying live-mode Stripe checkout. Remove once done.
  if (req.query && req.query.cleanup_test === process.env.ADMIN_SECRET) {
    try {
      await query(`DELETE FROM program_purchases WHERE agreement_signature_id IN (5, 6, 7)`);
      await query(`DELETE FROM agreement_signatures WHERE id IN (5, 6, 7)`);

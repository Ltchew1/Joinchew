...
| Notifications | Two jobs. `send-recommendation-reminders.js` is now cron-wired (`vercel.json`, hourly check, existing 24h/3d/7d thresholds unchanged) as of the Recommendation Engine closeout pass. `send-membership-reminders.js` remains NOT cron-wired — reclassified as required before the first client reaches the Membership transition (paid engagement → completion → 30-day Continuity → optional Membership), not before initial launch, since no client can reach that transition before completing a full engagement. Neither job reads any intelligence state. | TBD | TBD | TBD | TBD | TBD | TBD | TBD |
